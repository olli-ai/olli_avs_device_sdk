...
#define SENSORY_DEVICE "/dev/sensory-ww0"
...
u_int32_t *securityChipComms(u_int32_t *challenge)
{
    ...
    cmd.data[i++] = challenge[0] >> 16 & 0xff;
    cmd.data[i++] = challenge[0] >> 24 & 0xff;
    cmd.data[i++] = challenge[1] >> 0 & 0xff;
    cmd.data[i++] = challenge[1] >> 8 & 0xff;
